...
namespace ConsoleApp1
{
    class GoldRushMatchMaking
    {
        ...
        public GoldRushGameContainer gameContainer = new GoldRushGameContainer();
        Dictionary<String, int> iptoplayernum = new Dictionary<String, int>();
        private bool added;
        int id = 0;


        public GoldRushMatchMaking() { }

        public void AddMatch(String map_name, String ip)
        {
            added = false;
            ...
            {
                if (matchmaking_matches[i].map_name.Equals(map_name))
                {
                    ...
                    if (matchmaking_matches[i].Full())
                    {
                        ...
                        gameContainer.AddGame(level_num, 1, "normal", id++, players);
                    }
                }
            }
            ...
            {
                //maxplayernumber = 1;
                matchmaking_matches.Add(new Match(map_name, "normal", 2, "normal", "normal"));
        ...
        public void RemoveFromGame(int id, int player_id, String ip)
        {
            for (int i = running_matches.Count - 1; i >= 0; i--)
            {
                if (running_matches[i].players.Contains(ip))
                {
                    running_matches.RemoveAt(i);
                }
            }
            gameContainer.RemoveFromGame(id, player_id);
        }

        public void RemoveGame(int id, String ip)
        {
